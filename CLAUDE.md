# slk — agent rules

## Authority
- Architecture, layout, conventions, decisions → **[SPEC.md](SPEC.md)** (SSOT).
- Human-facing usage → [README.md](README.md) (onboarding + knowledge `--help` can't give; the complete command reference is `slk --help` / the generated `skills/` tree — do not duplicate the command tree here).
- Agent-facing skill prompt → the generated `skills/` tree: a small `slk` index ([skills/slk/SKILL.md](skills/slk/SKILL.md)) linking to `slk-shared` + one `slk-<group>` skill per group (generated by `slk generate-skills` from the command tree; do not hand-edit).
- Original design + plan → `docs/superpowers/specs/2026-05-19-slack-cli-design.md` and `docs/superpowers/plans/2026-05-19-slk-slack-cli.md`.

This file is index-only; do not duplicate SPEC.md content here.

## Run / build
```bash
go build -o slk ./cmd/slk                   # version comes from the committed VERSION file
go test ./...                               # uncached → prefix with go clean -testcache
go test ./internal/commands/ -run TestX -v  # focused
# release: bump VERSION + `go run ./cmd/slk generate-skills` + `./scripts/sync-version.sh` in a PR; merge to main → GHA auto-tags v<VERSION> & releases
goreleaser release --snapshot --clean       # local dry-run only
```

## Conventions
- Code comments + test names: English. See SPEC.md `## Conventions`.
- Doc language: SPEC.md / CLAUDE.md / this file in English; README.md mixes per audience.
- One file per command group under `internal/commands/`, paired `_test.go` with dry-run + flag-registration checks.
- Response-parsing helpers (`parseListCreateID`, `messageDisplay`, `injectRowID`, `fetchChannelsWith`, …) live alongside the command and are unit-tested via `httptest.NewServer` against `api.Client` with `BaseURL` overridden.
- Write verbs MUST honor `--raw` (return raw Slack JSON when set) and `--dry-run` (print the about-to-fire call and return without hitting the API).
- Exit codes: `0` ok · `3` auth · `4` not found · `5` rate-limited · `1` other. Mappings in `internal/api/errors.go`.

## Workflow rules
- Test fixtures and example identifiers use the scrubbed convention: `Alice` / `Bob` / `C0123456789` / `U0123456789` / `F01234567`. Do NOT introduce real names or real channel/user IDs into committed code or docs.
- When dispatching subagents for code work, explicitly tell them: "Do NOT run any commit-helper or documentation skill. Do NOT create SPEC.md, CLAUDE.md, or top-level README.md."
- Tokens live in `~/.config/slk/config.toml` (mode 0600); sensitive fields are encrypted at rest (AES-256-GCM), key in OS keyring or `~/.config/slk/.encryption_key` per `SLK_KEYRING_BACKEND` (`auto` default). Never print or log token strings or the encryption key. Details in SPEC.md.
- `VERSION` is the version SSOT. After changing it you MUST run BOTH `go run ./cmd/slk generate-skills` (rebuilds the `skills/` tree; guarded by CI `skill` job) and `./scripts/sync-version.sh` (fans into `gemini-extension.json` + `SECURITY.md`; guarded by CI `version-sync` job), then commit. Binary/tag/npm derive automatically; `npm/package.json`'s `0.0.0` is a publish-time placeholder — do NOT hand-bump. Fan-out details in SPEC.md.
- The command tree also drives generated artifacts independent of `VERSION`: the `skills/` tree and the README app-manifest scope block (`go run ./cmd/slk generate-manifest`). Regenerate BOTH after adding/removing a command or changing a command's `userScopes` / `botScopes` / `botCapable` annotations; each is CI-guarded (`skill` / `manifest` jobs). `generate-manifest` is scope-driven, NOT version-driven — it is not part of the VERSION-bump fan-out. Details in SPEC.md.
- Release is VERSION-driven (`.github/workflows/release.yml`): changing `VERSION` on `main` triggers it; a gate derives `v<VERSION>`, skips if the tag exists, else auto-creates the tag and releases (goreleaser + npm + homebrew). The tag is an artifact, not the trigger; no manual `git tag`. Do NOT run `goreleaser release` (without `--snapshot`) locally against the real `origin` remote — it would create a partial release and push a partial Homebrew formula.
- After any change merges to `main`, ASK the user whether to cut a release, with a recommended semver bump: patch = fixes, minor = additive features (pre-1.0 keeps breaking changes as minor too), major = post-1.0 breaks — judged over ALL work accumulated on `main` since the last `chore(release):`. A `VERSION` bump is ALWAYS its own standalone `chore(release): X.Y.Z` PR containing only the bump + `go run ./cmd/slk generate-skills` + `./scripts/sync-version.sh` — never bundled with a feature/fix/refactor/docs change. It triggers the full release, so the timing is the user's call.

## Slack-side traps (documented in README + SPEC)
- `chat.deleteScheduledMessage` may return ok=true for schedules within ~5 min of `post_at` yet still fire.
- `chat.delete` on a self-DM returns `internal_error`; UI-only deletion.
- `slackLists.delete` is not in the public API — lists must be cleaned via the Slack UI.
- `slk api --params` nested objects must be pre-serialized JSON strings (form-urlencoded transport).
- Slackbot is special-cased: Slack returns `is_bot=false` for it, so user-list filters key off the literal ID `USLACKBOT`.
